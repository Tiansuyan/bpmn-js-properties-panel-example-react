...
export default class PropertiesView extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <a onClick={() => saveXml(modeler)}>下载</a><br/>
        {
...
function saveXml (modeler) {
console.log('sjdkahfjksahkf');
console.log(modeler);
...
function ElementProperties(props) {
  ...
  function updateName(name) {
    ...
    console.log('modeling0000000000');
    console.log(modeling);
    console.log(modeling.updateLabel);

    modeling.updateLabel(element, name);
  }

  function updateTestValue(value) {
    const modeling = modeler.get('modeling');
  ...
  return (
    <div className="element-properties" key={ element.id }>
      ...
      <fieldset>
        ...
        <input value={ element.businessObject.get('testValue') } onChange={ (event) => {
              updateTestValue(event.target.value)
            } } />
      </fieldset>
